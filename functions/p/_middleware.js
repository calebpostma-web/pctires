...
//
// Route: functions/p/_middleware.js  ->  runs for every request under /p/
//        (pid == the feed `id` column, taken from the path).
//
// Deliberately a _middleware.js and not a [pid].js dynamic route: the deploy
// path is push-pctires.ps1, which PUTs each file to the GitHub Contents API by
// URL, and a filename with [ ] in it is not safely representable there. The
// /q/ quote links already use this exact pattern, so it is proven on this repo.
...
function renderProduct(row, meta, origin) {
  ...
  const canonical = origin + '/p/' + encodeURIComponent(row.id);
...
export async function onRequest(context) {
  ...
  // /p/<id>  ->  everything after the /p/ prefix, first segment only.
  let pid = url.pathname.replace(/^\/p\/?/, '').split('/')[0] || '';
  try { pid = decodeURIComponent(pid); } catch (e) { /* leave as-is */ }
  pid = String(pid).trim().replace(/\.html?$/i, '');

  // Bare /p or /p/ isn't a product -- send them to the catalogue.
